    ...
    def _packet_in_handler(self, ev):
        ...
        if out_port != ofproto.OFPP_FLOOD:
            ...
            if src in self.qos_list:
                match = datapath.ofproto_parser.OFPMatch(in_port=inport, dl_src=haddr_to_bin(src),
                                                         dl_dst=haddr_to_bin(dst))
                actions = [datapath.ofproto_parser.OFPActionEnqueue(out_port, 1)]
                self.add_flow(datapath, match, actions)
            else:
                match = datapath.ofproto_parser.OFPMatch(in_port=inport, dl_dst=haddr_to_bin(dst))
